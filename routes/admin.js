...
router.post('/admin-login', async (req, res) => {
    ...
    if(!userName && !password) {
        req.flash('adminLoginError', "Tizimga kirishi uchun Login va Parolni Kiritish Shart!")
    ...
    req.flash('adminLoginSuccess', `Xush Kelibsiz ${userName}! Qanday yangiliklar kiritamiz?`)
    res.redirect(`/admin-dashboard/${existAdmin._id}`)
})

router.post('/add-student',  async (req, res) => {
    const {firstName, surName, phoneNumber, password, role, gender, birthDate} = req.body
    ...
    req.flash('addStudentSuccess', "O'quvchi Muvaffaqiyatli Ro'yxatdan O'tkazildi!")
    res.redirect('/add-student')
})

router.post('/add-teacher',  async (req, res) => {
    const {firstName, surName, phoneNumber, password, role, gender, subject, birthDate} = req.body
    const hashedPass = await bcrypt.hash(password, 10)
    const teacherData = {
        firstName,
        surName,
    ...
    const token = generateJWTToken(data._id)     
    req.flash('addTeacherSuccess', "O'qituvchi Muvaffiyatli Ro'yxatdan O'tkazildi!")   
    res.redirect('/add-teacher')
})

router.post('/new-course', async (req, res) => {
    const {subject, level, lessonTime, lessonDay, teacherName, coursePrice} = req.body
...
router.post('/admin-update/:id', async (req, res) => {
    ...
    await Admin.findByIdAndUpdate(id, {password: hashedPassword}, {new: true})
...
router.get('/delete-user/:id', adminMid, async (req, res) => {
    ...
    try {
        ...
        return res.status(404).send('User not found in Student or Teacher collection.');
    } catch (error) {
